<?php
    ...
    if (isset($_POST['login'])) {
        ...
        if ($cekemail > 0) {
            ...
            if ($cekemail['email'] == $email AND $cekemail['password'] == $password) {
                ...
                if ($cekemail['id_level'] == 1) {
                    ...
                    echo "<script>alert('Berhasil login!'); document.location.href = '../views/superadmin';</script>";
                } else {
                    echo "<script>alert('Email atau password admin salah!'); document.location.href = '../views/auth/?login-admin';</script>";
                }
            } else {
                echo "<script>alert('Password yang dimasukkan salah!'); document.location.href = '../views/auth/?login-admin';</script>";
            }
        } else {
            echo "<script>alert('Email tidak ditemukan!'); document.location.href = '../views/auth/?login-admin';</script>";
        }
        ...
        if ($query) {
            ...
            header('location:../views/auth/login-admin.php');
    }